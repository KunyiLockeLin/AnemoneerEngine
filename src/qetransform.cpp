...
QeVector3f QeTransform::localFaceVector() { return MATH->eulerAnglesToVector(localFaceEular); }
    ...
    if (targetAnimationOID) {
        ...
        if (animation) {
            return animation->getBoneTransfrom(targetBoneName) *
                   MATH->getTransformMatrix(localPosition, localFaceEular, localScale, bRotate, bFixSize);
        }
    }
