...
void QeVulkan::initialize() {
    ...
    createInstance();
    setupDebugCallback();
    surface = createSurface(UI->mainWindow, UI->windowInstance);
    pickPhysicalDevice();
    createLogicalDevice();
    ...
    VK->createImage(emptyImageCube, 1, 6, size, VK_FORMAT_R8G8B8A8_UNORM, nullptr);
}

VkResult QeVulkan::CreateDebugReportCallbackEXT(VkInstance instance, const VkDebugReportCallbackCreateInfoEXT *pCreateInfo,
...
void QeVulkan::DestroyDebugReportCallbackEXT(VkInstance instance, VkDebugReportCallbackEXT callback,
...
void QeVulkan::createInstance() {
    ...
    std::vector<std::string> vs = COM_ENCODE.split<std::string>(node->getXMLValue<const char *>("applicationVersion"), ".");
    ...
    vs = COM_ENCODE.split<std::string>(node->getXMLValue<const char *>("engineVersion"), ".");
    appInfo.engineVersion = VK_MAKE_VERSION(atoi(vs[0].c_str()), atoi(vs[1].c_str()), atoi(vs[2].c_str()));

    vs = COM_ENCODE.split<std::string>(node->getXMLValue<const char *>("VulkanAPIVersion"), ".");
...
void QeVulkan::updatePushConstnats(VkCommandBuffer command_buffer) {
    ...
    if (size > 0)
        vkCmdPushConstants(command_buffer, pipelineLayout, VK_SHADER_STAGE_ALL, 0, uint32_t(size * sizeof(float)),
                           pushConstants.data());
}
...
VkFramebuffer QeVulkan::createFramebuffer(VkRenderPass renderPass, VkExtent2D size, std::vector<VkImageView> &views) {
    ...
    // for (int i = 0; i < VkImageViewLength; ++i)	attachments[i] = va_arg(keys,
    // const VkImageView); va_end(keys);
...
VkFormat QeVulkan::findSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling,
...
VkFormat QeVulkan::findDepthStencilFormat() {
    return findSupportedFormat({VK_FORMAT_D16_UNORM_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT, VK_FORMAT_D32_SFLOAT_S8_UINT},
                               VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
}

/*bool QeVulkan::hasStencilComponent(VkFormat format) {
        return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format ==
VK_FORMAT_D24_UNORM_S8_UINT || format == VK_FORMAT_D16_UNORM_S8_UINT;
}*/

void QeVulkan::transitionImageLayout(VkCommandBuffer cmdBuf, QeVKImage &image, VkImageLayout newLayout, int imageCount,
                                     uint32_t mipLevels) {
    ...
    if (newLayout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL) {
        ...
        // if (hasStencilComponent(format))	barrier.subresourceRange.aspectMask |=
    ...
    switch (image.layout) {
        ...
        case VK_IMAGE_LAYOUT_GENERAL:
            ...
            break;
        default:
            LOG("unsupported src layout transition!");
            break;
    }
    switch (newLayout) {
        case VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL:
        ...
        case VK_IMAGE_LAYOUT_GENERAL:
            ...
            break;
        default:
            LOG("unsupported dst layout transition!");
            break;
    }
...
VkExtent2D QeVulkan::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
    if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
        ...
        UI->getWindowSize(UI->mainWindow, width, height);
...
VkPipeline QeVulkan::createGraphicsPipeline(QeDataGraphicsPipeline *data) {
    ...
    while (it != graphicsPipelines.end()) {
        if ((*it)->shader->vert == data->shader->vert && (*it)->shader->tesc == data->shader->tesc &&
            (*it)->shader->tese == data->shader->tese && (*it)->shader->geom == data->shader->geom &&
            (*it)->shader->frag == data->shader->frag && (*it)->renderPass == data->renderPass && (*it)->bAlpha == data->bAlpha &&
            (*it)->componentType == data->componentType && (*it)->minorType == data->minorType && (*it)->subpass == data->subpass &&
    ...
    if (bVertex) {           // bVertex
        std::string s = "";  // Magic line!!!! In release mode, if it's removed,
                             // attributeDescriptions would become bad value and shutdown.
    ...
    if (data->bAlpha) {
        ...
        colorBlendAttachment.dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE;  // VK_BLEND_FACTOR_ZERO;//
...
VkPipeline QeVulkan::createComputePipeline(QeDataComputePipeline *data) {
    ...
    VkPipeline pipeline;
    if (VK_SUCCESS != vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &createInfo, nullptr, &pipeline))
...
/*void QeVulkan::updateDescriptorSetRayTracing(QeDataDescriptorSetRaytracing&
descriptor, QeDataDescriptorSet& descriptorSet) {
...
/*void QeVulkan::generateMipmaps(VkImage image, int32_t texWidth, int32_t
...
void QeVulkan::createImage(QeVKImage &image, VkDeviceSize dataSize, int imageCount, VkExtent2D &imageSize, VkFormat format,
                           void *data, VkSampleCountFlagBits sampleCount) {
    uint32_t mipLevels = 1;  // static_cast<uint32_t>(std::floor(std::log2(std::max(width,
                             // height)))) + 1;
    ...
    VkImageLayout layout_src = VK_IMAGE_LAYOUT_UNDEFINED;
    VkImageLayout layout_dst = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    int arrayLayers = 1;
    bool bImage = true;
    bool bMemory = true;
    bool bView = true;
    ...
    switch (image.type) {
        case eImage_depthStencil:
            ...
            layout_dst = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
            bLayout = true;
            break;
    ...
    if (bImage) {
        ...
        imageInfo.initialLayout = layout_src;  // VK_IMAGE_LAYOUT_PREINITIALIZED;
    ...
    if (bSampler) {
        ...
        samplerInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;              // VK_SAMPLER_MIPMAP_MODE_LINEAR;//
        ...
        samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;          // VK_BORDER_COLOR_FLOAT_OPAQUE_BLACK;//
        ...
        if (vkCreateSampler(VK->device, &samplerInfo, nullptr, &image.sampler) != VK_SUCCESS)
            LOG("failed to create texture sampler!");
    }

    if (data) {
        transitionImageLayout(VK_NULL_HANDLE, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, imageCount /*, mipLevels*/);
        ...
        copyBufferToImage(staging.buffer, image.image, dataSize, imageCount, imageSize /*, layer*/);
        layout_src = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
    }
    if (bLayout) transitionImageLayout(VK_NULL_HANDLE, image, layout_dst, imageCount /*, mipLevels*/);
