...
QeAssetModel *QeEncode::decodeGLTF(QeAssetJSON *json, bool bCubeMap) {
    ...
    */
    ...
    if (c) {
        ...
        if (bCubeMap)
            pMaterial->image.pCubeMap = AST->getImage(texturePath, bCubeMap, true);
        else
            pMaterial->image.pBaseColorMap = AST->getImage(texturePath, bCubeMap, true);
    }
...
void QeEncode::getHuffmanDecodeSymbolfromDCAC(short int *out, unsigned char blocks, const unsigned char *in, size_t *bitPointer,
...
void QeEncode::buildDynamicLZ77HuffmanTree(QeHuffmanTree *treeLL, QeHuffmanTree *treeD, const unsigned char *in,
                                           size_t *bitPointer) {
    ...
    for (i = 0; i < 19; ++i)
        if (i < HCLEN) bitlenCCL[CCL_ORDER[i]] = readBits(in, bitPointer, 3);
...
void QeEncode::decodeLitLenDis(std::vector<unsigned char> *out, QeHuffmanTree *treeLL, QeHuffmanTree *treeD, unsigned char *in,
                               size_t *bitPointer) {
    while (1) {
