...
vec3 Diffuse_Lambertian(vec3 baseColor) { return baseColor / M_PI; }
...
float GK_IBL(float roughness) { return (roughness * roughness) / 2.0; }
...
vec3 F_FresnelSchlick(float dotVH, vec3 F0) { return (F0 + (1.0 - F0) * pow(1.0 - dotVH, 5.0)); }
